...
namespace CosmeticShop.UI.Areas.Admin.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        public async Task<IActionResult> Edit(EditProductViewModel model, IFormFile image)
        {
            ...
            ViewBag.CategoryErrorMessage = model.CategoryIds.Count == 0 ? "En az bir adet kategori seçmeniz gerekiyor." : null;
            var categories = await _categoryManager.GetActiveCategories();
            model.Categories = categories.Data;
            return View(model);
        }
        ...
        public async Task<IActionResult> SoftDelete(int id)
        {
            ...
            if (productViewModel.Data.IsDeleted)
                _notyfService.Success("Ürün çöp kutusuna atılmıştır.");
            else
                _notyfService.Success("Ürün geri alınmıştır.");
